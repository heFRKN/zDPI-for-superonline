...
title zDPI - Tek Seferlik (Mod 4 (TTL 4 Turkcell))
...
echo.
echo -----------------------------------------------------------------------
echo  [+] zDPI Tek Seferlik Mod Aktif (Mod 4 (TTL 4 Turkcell))
echo  [+] Bu pencere ACIK KALDIGI SURECE Discord ve tum siteler aciktir.
echo  [+] Kapatmak icin pencereyi kapatin veya Ctrl+C yapin.
echo -----------------------------------------------------------------------
echo.

winws.exe --wf-tcp=80,443 --wf-udp=443,50000-65535 --filter-udp=443,50000-65535 --dpi-desync=fake --dpi-desync-ttl=4 --new --filter-tcp=80,443 --dpi-desync=fake,split2 --dpi-desync-split-pos=2 --dpi-desync-ttl=4 --dpi-desync-fooling=badseq
